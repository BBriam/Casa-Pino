<div class="malla_3_main tamf_16 nega5 morado">

<article class = "malla_1_categoria">
    <a class="prod_item" href="detalle.php?c=17&s=001&p=001">
      <img class="img_fondo" src="static/productos/prod17_001.png">
      <div class="overlay">
        <i class="fa fa-search lupa"></i>
      </div>
    </a>
    <a class="tamf_16 nega5 morado" href="detalle.php?c=17&s=001&p=001">
      <p class="prod_precio tamf_24 gris51">$317.00</p>
      <p class="prod_nombre">Pino amarillo</p>
      <!-- <p class="prod_nombre">Shampoo Pediculosis x 350 ml Zapping</p> -->
    </a>
</article>

<article class = "malla_1_categoria">
    <a class="prod_item" href="detalle.php?c=17&s=001&p=002">
      <img class="img_fondo" src="static/productos/prod17_002.png">
      <div class="overlay">
        <i class="fa fa-search lupa"></i>
      </div>
    </a>
    <a class="tamf_16 nega5 morado" href="detalle.php?c=17&s=001&p=002">
      <p class="prod_precio tamf_24 gris51">$673.00</p>
      <p class="prod_nombre">Palmera</p>
    </a>
</article>

<article class = "malla_1_categoria">
    <a class="prod_item" href="detalle.php?c=17&s=001&p=003">
      <img class="img_fondo" src="static/productos/prod17_003.png">
      <div class="overlay">
        <i class="fa fa-search lupa"></i>
      </div>
    </a>
    <a class="tamf_16 nega5 morado" href="detalle.php?c=17&s=001&p=003">
      <p class="prod_precio tamf_24 gris51">$168.00</p>
      <p class="prod_nombre">Arbusto tipo pino</p>
    </a>
</article>

</div>